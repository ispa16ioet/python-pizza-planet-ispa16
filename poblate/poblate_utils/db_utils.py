...
def insert_ingredient_detail(cursor: Any, order_detail: int, order_id: int) -> None:
    cursor.execute(
        """
        INSERT INTO `ingredient_detail` (`order_id`,`ingredient_id`, `ingredient_price`)
        VALUES (?, ?, ?)
    """,
        (order_id, order_detail["ingredient_id"], order_detail["ingredient_price"]),
    )


def insert_beverage_detail(cursor: Any, order_detail: int, order_id: int) -> None:
    cursor.execute(
        """
        INSERT INTO `beverage_detail` (`order_id`, `beverage_id`, `beverage_price`)
        VALUES (?, ?, ?)
    """,
        (order_id, order_detail["beverage_id"], order_detail["beverage_price"]),
    )
